		...
		<view style="border-bottom: 6px solid #EEEEEE;">
			<view style="display: flex;justify-content: center;align-items: center;margin-top: 20rpx;margin-bottom: 20rpx;margin-left: 20rpx;flex-direction: column;margin-right: 20rpx;">
				...
				<view style="display: flex;justify-content: space-between;width: 100%;margin: 20rpx 0;">
						...
						<view style="display: flex;">
							<view style="display: flex;flex-direction: column;justify-content: flex-start;line-height: 40rpx;">
								<text style="color: #818181;font-size: 26rpx;">购彩金</text>
								<text style="color: red;font-size: 32rpx;">{{user.now_money}}</text>
							</view>
							<view style="display: flex;flex-direction: column;justify-content: flex-start;line-height: 40rpx;margin-left: 20rpx;">
								<text style="color: #818181;font-size: 26rpx;">奖池</text>
								<text style="color: red;font-size: 32rpx;">{{user.award_amount}}</text>
							</view>
						</view>
		...
		<view style="padding: 40rpx 20rpx;">
			...
			<view style="display: flex;justify-content: space-between;margin-bottom: 20rpx;">
				...
				<view>
					...
					<checkbox class='round red sm zaiui-checked' :class="change_mode==1?'checked':''"
					 :checked="change_mode==1?true:false" value="1"/>
					 <label>购彩金</label>
					 <checkbox class='round red sm zaiui-checked' :class="change_mode==2?'checked':''"
					  :checked="change_mode==2?true:false" value="2"/>
					  <label>奖池</label>
					</checkbox-group>
				</view>
			</view>
...
<script>
	...
	export default{
		...
		methods:{
			CheckboxChange(){
				this.change_mode = this.change_mode==1?2:1;
			},
			deleteUser(uid){
				uni.showModal({
					title: '提示',
			...
			changeMoney(){
				if(this.money){
					if(parseFloat(this.money)!=0){
						...
						var data = {"uid":this.user.uid,"value":this.money,"mark":this.mark,"change_mode":this.change_mode}
